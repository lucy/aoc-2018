...
int main(void) {
	char scan[2000][1000] = {0}, c;
	int ymin = 2000, ymax = 0, xmin = 1000, xmax = 0;
	for (;;) {
		int y1, y2, x1, x2;
		if (scanf("y=%d, x=%d..%d\n", &y1, &x1, &x2) == 3) y2 = y1; else
		if (scanf("x=%d, y=%d..%d\n", &x1, &y1, &y2) == 3) x2 = x1; else break;
		for (int x = x1; x <= x2; x++) for (int y = y1; y <= y2; y++)
			if (y >= 2000 || x >= 1000) return 1; else scan[y][x] = '#';
		if (y1 < ymin) ymin = y1;
		if (y2 > ymax) ymax = y2;
		if (x1 < xmin) xmin = x1;
		if (x2 > xmax) xmax = x2;
	...
	while (i) {
		...
		/* if unsupported (doesn't have '#'/'~' below), skip */
		...
		/* if row of '|' is touching '#'s on both sides, turn into '~' */
		for (; x1 <= x2; scan[y][x1++] = '~')
			/* push all '|'s above '~' row */
